...
export default function Glances({ glances }) {
  ...
  function navigateGlance(direction = 1) {
    ...
    router.push(`/sodrek?glanceSlug=${newSlug}`, `/sodrek/${newSlug}`, {
      shallow: true,
    });
  }
  ...
  return (
    <Wrapper>
      ...
      {isScrollActive ? (
        ...
        <div className="grid grid-cols-3 mt-4 gap-1 md:gap-6 -mx-4 md:mx-0">
          {glances.map((glance) => (
            <Link
              key={glance.slug}
              href={`/sodrek?${glanceLinkParam}=${glance.slug}`}
              as={`/sodrek/${glance.slug}`}
              shallow={true}
            >
      ...
      <Modal
        isOpen={isModalOpen}
        onRequestClose={() => router.push("/sodrek")}
      ...
      >
        <Glance
          ...
          glance={glances.find(
            (glance) => glance.slug === router.query.glanceSlug
          )}
        />
      </Modal>
    </Wrapper>
